...
Each receptor neuron expresses only one kind of olfactory receptor (in insects they are co-expressed with Orco \cite{Larsson2004}).
...
Both terms are characteristic of each OR, and they might vary from neuron to neuron.
...
\end{equation}
where, $v_n$ is the preferred molecular volume of odorant receptor $n$ and $\sigma_n$ represents its flexibility. 
...
If we use a simple threshold of 0.05 for the p-value of each OR, we may have false positives. 
...
We also want to show the diversity of volume and flexibility of binding pocket among ORs.
To estimate the p-values, 
we take any pair of ORs that was sensitive to molecular volume (\numberofreceptors ORs),
