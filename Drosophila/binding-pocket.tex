...
\title{Odorant receptors of Drosophila are sensitive to molecular volume of odorants}
...
yet to be answered. Human olfactory system has a repertoire of about 350 odorant receptors. 
...
\begin{figure}
	...
	\caption{This figure shows different scenarios that may happen when an odorant molecule (ligand) binds to a receptor. 
		...
		and the blue shapes are odorant receptor and binding-pocket.	
...
The front end of the olfactory system are odorant receptor neurons.  
Each receptor neuron expresses only one kind of odorant receptor (in insects many are co-expressed with Orco \cite{Larsson2004}).
Neurons of the same type converge into the same glomeruli of the olfactory bulb (or antenatal lobe in insects),
so that each glomerulus of olfactory bulb receives an amplified signal from only one type of odorant receptor~\cite{root2007,Carey2011,Vosshall2000,Couto2005,fishilevich2005,gao2000,wang1998,mombaerts1996,vassar1994}.
...
%whereas electro-physiological studies directly target odorant receptor neurons (eg.~\cite{Benton2010}). 
...
an odorant receptor can be triggered by many odorant molecules, 
and an odorant molecule can interact with different odorant receptors~\cite{Malnic2000},
...
In this study, 
we investigated the relation between molecular volumes of odorants and the responses of odorant receptor neurons. 
Our results suggest that molecular volume is a considerable factor, 
but not the only factor that determines the neural response of the odorant receptor neurons.

The odorant receptors are transmembrane proteins.
...
But recently, it has been argued that the odorant receptors in insects are ionotropic~\cite{Sato2008,Wicher2008,Nagel2011,Rong2011}, 	
...
all odorant receptor have the same function, they have a binding-pocket (also known as binding-cavity and binding-site),
...
The amount of change in the membrane potential of an odorant receptor neuron depends on the number of activated odorant receptors and the time that they remain activated,
...
In this study, we predict the {\it in-vivo} volume and flexibility of binding-pocket of odorant receptors, by applying our mathematical method to neural data from DoOR database~\cite{Galizia2010}. 

We suggest a functional relation between molecular volume and the neural responses, 
we provide a methodology to estimate {\it molecular receptive range} or {\it tuning function} of odorant receptors,
and then we predict the structural properties of the binding-pocket of odorant receptor - the volume and the flexibility of binding-pocket.
Our results may help to select odorants  for new experimental studies, 
may provide additional information about the structure of odorant receptors to structural biologists, 
...
that includes the neural responses of most odorant receptors (OR) of \textit{Drosophila} to many odorants~\cite{Galizia2010}. 
...
``{\it The response of odorant receptor neurons depend on the molecular volume of odorants.}'', 
which required  a finite value for $\sigma_n$, 
then the null hypothesis was 
``{\it The response of odorant receptor neurons is independent of molecular volume of odorants.}'',
...
%	\end{subfigure}
	\caption{Response of odorant receptors  versus molecular volume of odorants (circles),  
...
% do not touch this fig.
	...
	\caption{Pairs of odorant receptors that differ significantly in their binding-pocket's volume (upper triangle) and flexibility (lower triangle).
...
First we assumed that the response of an odorant receptor can be factorized into two terms, 
...
The function $f_n(v)$ can be considered as the tuning curve of odorant receptor $n$ in response to molecular volume (Fig.~\ref{fig:vol-res}). 
...
If all odorant receptors had the same preferred volume and flexibility, 
any change in the molecular volume would change only the intensity of smell not its quality.
But odorant receptors have different preferred volumes and flexibilities, 
...
Here we showed that the responses of odorant receptor neurons are related to the molecular volume of odorants, 
apart from that, it is not clear which other features of molecules are measured by odorant receptors. 
...
We also predict some {\it in-vivo} structural aspects of  the binding-pocket of odorant receptors:
...
These data add some constrains over the 3d structure of odorant receptors, 
...
Some genes of an odorant receptor are mutated, 
...
In this way we can understand which amino acids of the odorant receptor contribute to the volume and flexibility of the binding-pocket, 
...
\centering
	...
	\caption{Venn diagram of DoOR database and our suggested important odorants of each OR.
			...
			some are used to study an odorant receptor (blue areas), 
