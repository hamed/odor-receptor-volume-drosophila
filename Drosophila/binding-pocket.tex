...
ORN of the same kind converge into the same glomeruli of antennal lobe  in insects (or olfactory bulb in vertebrates), 
...
We shuffled the association between rodorants and responses of a given OR,  
...
% do not touch this fig.
	\centering
	\includegraphics[width= 0.75 \textwidth]{pair-pval}
	\caption{Or82a did not confirem our theory (left), but when Geranyl acetate removed from the data it confirmed our model with a bonfrreni corrected p-value of 0.03 (right).}
	\label{fig:p-values}
\end{figure}
One interesting case in this regard was Or82a, that did not fit to our theory. 
It specifically binds to Geranyl acetate and it is not very sensitive to other molecules. 
But we removed Geranyl acetate from the data and suddenly Or82a become a perfect fit with a Bonferroni corrected p-value of 0.03. 
This effect can be due to long and some how rigid chain of Geranyl acetate that can not be modeled as an spherical shape, 
simply by considering its volume. 
