...
Following this fact, we predicted the actual \textit{in-vivo} volume and flexibility of the binding-pockets of ORs, 
the results are in supplemental file volume-profiles.csv and they can be verified when the 3D structure is resolved, 
or more experiment are done. 

Now that we know how much of the variance in the response of ORN was resulted from molecular volume, 
it is possible to study the effect of other parameters, we hope this advances the field of olfaction. 

Both the model and the data can be improved in many days. 
For example, 
the data can benefit from more controls on parameters like concentration, 
vapor pressure and solubility in water, 
also the effect of sensillum lymph and its odor binding-proteins might be important which are not addressed in the current experimental methods. 
Using experimental paradigm like Luciferase Assay as used to study human olfactory systems \cite{human olf}, 
may provide valuable complementary information. 

Also many improvement can be made on the model.
Here we did not consider anisotropy of molecules
