...
so that each glomerulus of olfactory bulb receives an amplified signal from only one type of olfactory receptor.
...
for the discovery of this and some of the above facts.
Scientist believed that insects use the same kind of signal transduction, 
but recently, XX and XX argued that the olfactory receptors in insect are inotropic. 
...
The amount of change in the membrane potential depends on the number of activated receptors and the time that they remain activated.
These are affected by various physio-chemical properties of ligand and receptor, 
...
But this is not the case here, it is not easy to know the structure of integral proteins in general, 
and the 3d structure of olfactory receptors is the topic of ongoing researches, 
using various methods like Molecular Dynamic simulations (MD), 
mutagenesis studies, heterologus expression studies, and homology modeling.
...
that includes the neural responses of most olfactory receptors (OR) of Drosophila to many odorants. 
...
Considering the binding-process (Fig. \ref{fig:binding-pocket}), and the neural responses \ref{fig:vol-res}.
Although these assumptions are logical, we also put them in a test. 
...
Each receptor has a preferred molecular volume $v_n$ and receptors can show different degree of selectivity $\sigma_n$.
