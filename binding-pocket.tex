...
it is a topic of ongoing researches and there are many theories~\cite{Turin,Keller2004,Araneda2000,Brookes2007,Franco2011,Pelz2006,Gabler2013,Schmuker2007,Haddad2008,Snitz2013,Yablonka2012,gane2013,boyle2013}.
...
which are determined by various physio-chemical properties of the ligand (odorant) and the receptor~\cite{Turin,Araneda2000,Gabler2013,boyle2013,zarzo2011}, 
...
so p-value is the probability of having $\sigma'_n<\sigma_n$, 
where $\sigma_n$ is calculated from the original data, but $\sigma'_n$ calculated using permuted version.

We are testing a hypothesis 60 times for we have 60  olfactory receptors. 
If we use a simple threshold of 0.05 for the p-value, we may have false positives. 
To address this issue, we use a conservative threshold of 0.05/60 for the p-values -- Bonferroni correction.
...
We used both methods -- Bonferroni and FDR, as well as no correction. The result are labeled accordingly in Fig.\ref{fig:vol-res:all}.
...
%	\end{subfigure}
	\caption{Response of \numberofreceptors olfactory receptors  versus molecular volume of odorants (circles),  
		the fitted functions $f_n(v)$ from Eq.~\ref{eqn:factors} (solid lines), 
		and the error bars of the mean of $f_n(v)$ (red vertical lines).
		It shows that response of receptors depend on the molecular volume of odorants,
		11 of them (receptor names in bold) has a p-value of less than 0.05/60 (Bonferroni correction), 
		tow of them (receptor name in italic) has a p-value of less than 0.05,
		and the rest have a FDR-corrected p-value of less than 0.05. %(b) Just an example (Or35a) to magnify details. 
		}
	\label{fig:vol-res}
\end{figure}
\begin{figure}
	\begin{subfigure}[b]{\textwidth}
		...
		Some receptors prefer smaller molecules - like Or33b, Or7a and  Or92a, 
		but some other receptors prefer larger molecules - like  Or85e,  Or74a and  Or59c.}
	...
	\begin{subfigure}[b]{\textwidth}
		...
		\caption{The flexibility of each receptor ($\sigma_n$), the error bars are calculated using Jack-Knife method.
			...
			but some other receptors like Or88a,  Or82a and  Or74a show flexibility and respond to broader range of molecular volumes.
		}
...
\cite{zarzo2011}There are two main assumption in this work: 
...
we can conclude that over one third of receptors (\fdr / 60 ) show significant sensitivity toward molecular volumes. 
...
In a hypothetical experiment, assume that every characteristic of odorant molecules are the same but their molecular volume.
...
We suggested the best selection of odorants for each of \numberofreceptors studied receptors (see Venn diagram in Fig.~\ref{fig:odorant-suggest} and supplemental file 2).
%\begin{table}
%\centering
