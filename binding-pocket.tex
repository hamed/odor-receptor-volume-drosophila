...
where $\sigma_n$ is calculated from the original data, but $\sigma'_n$ calculated using permuted data.

We are testing a hypothesis multiple times over 49 olfactory receptors. 
If we use a simple threshold of p-value < 0.05, we may have some false positives. 
To address this issue, we use a conservative threshold of 0.05/49, Bonferroni correction.
The problem with Bonferroni correction is that it may increases false negatives.
This problem can be addressed using another method to correct p-values - False Discovery rate.
...
\begin{figure}
	...
	\caption{(a) Response of \numberofreceptors olfactory receptors  versus molecular volume of odorants (circles),  
		...
		and the rest have a FDR-corrected p-value of less than 0.05. 
		(b) Just an example (Or35a) to magnify details. }
	\label{fig:vol-res}
\end{figure}
\begin{figure}
	\begin{subfigure}[b]{\textwidth}
